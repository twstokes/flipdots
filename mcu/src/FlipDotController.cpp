...
FlipDotController::FlipDotController(HardwareSerial *serial, int baud) : serial(serial), baud(baud)
{
}
...
/*
    writes the frame defined in the AlfaZeta documentation to the board controller
*/
...
/*
    ...
    if immediate is true, the panel will write out the buffer as soon as it is received
    if immediate is false, refreshDisplays() should be called eventually to commit all buffers
*/
void FlipDotController::sendBufferToPanel(byte address, const byte *data, bool immediate)
{
